...
# to pixels
def assign_features_to_pixels(xy, nn, verbose=False) -> np.ndarray:
    """Provides a mapping of which pixel represents which feature.

    For each unassigned feature, find its nearest pixel, repeat until every
    feature is assigned.

    Args:
        xy: The embedding 2-d coordinates (normalized to [0,1]).
        nn: image width to be generated
        verbose: Whether progress should be printed.

    Returns:
        An array of shape at most [nn x nn, 3], i.e., [number of pixels, 3].
        Contents of this array are [x, y, feature] where x, y are the pixel
        coordinates and feature is the feature number pixel x, y represents.
        Pixels which do not represent any feature are not found in this array.
    """
    Nn = xy.shape[0]
    ...
    while feature_assigned.sum() < Nn:
        ...
        for each_feature in range(Nn):
            ...
            if feature_assigned[each_feature] == True:
                ...
                for ii in range(nn * nn):
                    ...
                    nearest_pixel_idx = \
                    np.argsort(dist_xy_centroids[:, each_feature])[ii]
        ...
        for each_pixel in range(nn * nn):
            ...
            if pixel_avail[each_pixel] == False:
                ...
                related_features = \
                np.where(pick_xy_centroids[each_pixel, :] == 1)[0]
                if len(related_features) == 1:
                    ...
                    related_dists = dist_xy_centroids[
                        each_pixel, related_features]
                    best_feature = related_features[np.argsort(related_dists)[
                        0]]  # Sort, and pick the nearest one among them
                    result_table[each_pixel, 2] = best_feature
                    pixel_avail[each_pixel] = False
                    feature_assigned[best_feature] = True
        if verbose:
            print(">> Assign features to pixels:", feature_assigned.sum(), "/",
                  Nn)
    ...
    # result_table = result_table[result_table[:, 2] > 0]
    # return result_table
...
def REFINED_Im_Gen(X, nn, map_in_int, gene_names, coords):
    [N_sam, P_Feat] = X.shape
    X_Gen = np.zeros((N_sam, nn ** 2))
    for i in trange(N_sam):
        data = X[i, :]
        X_REFINED = pd.DataFrame(data=data.reshape(1, len(data)),
                                 columns=gene_names)
...
def GDSC_NPier(PD_Set, Set_Name, PD_Attribute, Attribute_Name):
    ...
    for name in Source:
        ...
        XX = np.array(
            PD_Attribute[PD_Attribute[Attribute_Name] == name].values[0, 1:],
            dtype=float)
        X_NP[idx, :] = XX
    return X_NP
