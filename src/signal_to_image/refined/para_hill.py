...
def universial_corr(dist_matr, mapping_in_int):
    ...
    # Because np.where returns a tuple (x_position array, y_position array),
    # a generation is used
    coord = np.array(
        [[item[0] for item in np.where(mapping_in_int == ii)] for ii in
         range(Nn)])
    ...
    L2_Norm = np.sqrt(
        sum((pixel_dist - feature_dist) ** 2) / sum(feature_dist ** 2))
    return L2_Norm
...
def evaluate_swap(coord1, coord2, dist_matr, mapping_in_int, original_corr=-2):
    ...
    if coord1[0] < 0 or coord1[1] < 0 or coord2[0] < 0 or coord2[1] < 0:
        return np.nan
    if coord1[0] >= the_map.shape[0] or coord1[1] >= the_map.shape[0] or coord2[
        0] >= the_map.shape[0] or coord2[1] >= the_map.shape[0]:
        return np.nan
    ...
    try:
        ...
        raise Warning("Swap index:", coord1, coord2,
                      "Index error. Check the coordnation.")
        return np.nan
...
def evaluate_centroid(centroid, dist_matr, mapping_in_int):
    ...
    for each_direc in product([-1, 0, 1], repeat=2):
        ...
        evaluation = evaluate_swap(centroid, swap_coord, dist_matr,
                                   mapping_in_int, original_corr)
...
def execute_coordination_swap(coord1, coord2, mapping_in_int):
    ...
    # If out of bound, return NaN.
    if coord1[0] < 0 or coord1[1] < 0 or coord2[0] < 0 or coord2[1] < 0:
        raise Warning("Swapping failed:", coord1, coord2,
                      "-- Negative coordnation.")
        return the_map
    if coord1[0] > the_map.shape[0] or coord1[1] > the_map.shape[0] or coord2[
        0] > the_map.shape[0] or coord2[1] > the_map.shape[0]:
        raise Warning("Swapping failed:", coord1, coord2,
                      "-- Coordnation out of bound.")
        return the_map
...
def execute_dict_swap(swapping_dict, mapping_in_int):
    for each_key in swapping_dict:
        execute_direction_swap(each_key, mapping_in_int,
                               direction=swapping_dict[each_key])
    return mapping_in_int